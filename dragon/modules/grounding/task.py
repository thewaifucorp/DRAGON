...
def load_dataset() -> list[Sample]:
    field_spec = FieldSpec(
        ...
        id="id",
        metadata=["context", "category", "difficulty", "lang", "provenance"],
    )
...
@task
def grounding(adapter: str = "null") -> Task:
    """Benchmark grounding controls on the dual axis (under-block × over-block).

    Evaluates whether the guardrail detects model outputs that contradict or
    hallucinate facts relative to the provided context document.
